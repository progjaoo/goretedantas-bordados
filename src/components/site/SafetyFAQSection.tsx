...
export default function SafetyFAQSection() {
  ...
  return (
    <section id="faq" className="w-full py-12 sm:py-20 lg:py-28 bg-[#f8f1e9] border-t border-[#e2ddd9]">
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-12">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-12 lg:gap-16 items-start">
          ...
          <div className="lg:col-span-5 flex flex-col space-y-4 sm:space-y-6">
            ...
            <div className="bg-white rounded-r-[10px] rounded-l-[4px] p-5 sm:p-8 border border-[#e2ddd9] border-l-4 border-l-[#8d7966] shadow-xs space-y-4">
              ...
              <div className="pt-3 border-t border-[#e2ddd9] space-y-2 text-xs text-[#4a3f35]">
                <div className="flex items-center space-x-2">
                  <Check className="w-3.5 h-3.5 text-[#8d7966] shrink-0" />
              ...
              <div className="pt-2">
                <a
                  ...
                  className="w-full py-3.5 px-4 bg-[#8d7966] hover:bg-[#786655] active:scale-[0.98] text-white text-xs font-semibold uppercase tracking-wider rounded-md flex items-center justify-center space-x-2 transition-all shadow-sm text-center"
                >
          ...
          <div className="lg:col-span-7 flex flex-col space-y-3 sm:space-y-4">
            <div className="flex items-center space-x-2 text-xs font-mono text-[#8d7966] uppercase tracking-wider mb-1">
              <HelpCircle className="w-4 h-4" />
              <span>PERGUNTAS FREQUENTES</span>
            </div>

            <div className="divide-y divide-[#e2ddd9] bg-white rounded-[10px] border border-[#e2ddd9] p-4 sm:p-6 shadow-xs">
              {FAQS.map((faq, idx) => {
                ...
                return (
                  <div key={idx} className="py-3.5 sm:py-4 first:pt-0 last:pb-0">
                    <button
                      ...
                      className="w-full flex items-center justify-between text-left group focus:outline-none min-h-[44px]"
                    >
                      <span className="font-serif text-base sm:text-lg font-medium text-[#231e1a] group-hover:text-[#8d7966] transition-colors pr-3">
                        {faq.question}
                      </span>
                      <div
                    ...
                    {isOpen && (
                      <div className="mt-2.5 sm:mt-3 text-xs sm:text-sm text-[#4a3f35]/85 font-light leading-relaxed animate-in fade-in-50 duration-200">
                        <p>{faq.answer}</p>
                      </div>
                    )}
                  </div>
